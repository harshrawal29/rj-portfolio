...
export default function Cursor() {
  ...
  useEffect(() => {
    ...
    const handleMouseOver = (e: MouseEvent) => {
      ...
      const interactable = target.closest('a, button, input, textarea, select, [role="button"], [data-cursor="view"]');
      if (interactable) {
        ...
        if (interactable.hasAttribute('data-cursor')) {
          ...
          if (textEl) gsap.to(textEl, { opacity: 1, color: 'white', duration: 0.3 });
    ...
    const handleMouseOut = (e: MouseEvent) => {
      ...
      if (interactable) {
        ...
        const textEl = cursor.querySelector('.cursor-text');
        if (textEl) gsap.to(textEl, { opacity: 0, duration: 0.3 });
      }
    };
  ...
  return (
    <>
      <div
        ref={cursorRef}
        className="fixed top-0 left-0 w-8 h-8 rounded-full border-[1.5px] border-white pointer-events-none z-[9999] mix-blend-difference flex items-center justify-center overflow-hidden"
        style={{ willChange: 'transform, width, height' }}
      >
        <span className="cursor-text text-[10px] font-semibold tracking-[0.2em] uppercase text-white opacity-0 whitespace-nowrap" style={{ transform: 'scale(0.4)' }}>
          View
        </span>
      </div>
      <div
